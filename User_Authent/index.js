...
app.post("/creat", async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({ message: "User Created Successfully",code:1 })
    } catch (err) {
        console.log(err)
        res.status(201).json({ message: "User Createation Failed " ,code:0 })
    }
})
app.post("/authent", async (req, res) => {
    ...
    const fetchQuery = `SELECT password,id FROM ${clint} WHERE gmail = $1`
    ...
    try {
        ...
        if (pwd === dbPassword) {
            return res.status(200).json({ message: "Match", code: 1 ,id:id})
        } else {
